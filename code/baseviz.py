...
def baseviz(idf_object, baseout_dir: str):
    ...
    m_col4.metric("Peak Design Occupancy", f"{total_people} People" if total_people > 0 else "Area-Based / Hidden")
    ...
    diag_tab1, diag_tab2, diag_tab3 = st.tabs([
            ...
            "🌡️ Material Thermal Properties"
        ])
        
    # --- TAB 1: WALL ORIENTATIONS ---
    with diag_tab1:
        st.write("#### Interactive Wall Orientation Profile")
        if not df_walls.empty:
            # Create interactive azimuth bins (30-degree increments for 360-degree compass layout)
            bins = range(0, 390, 30)
            df_walls['Azimuth_Bin'] = pd.cut(df_walls['Azimuth_deg'], bins=bins, labels=[f"{b}°-{b+30}°" for b in bins[:-1]])
            
            # Group surface areas cleanly for the bar chart
            orientation_chart = df_walls.groupby('Azimuth_Bin', observed=False)['Area_m2'].sum().to_frame()
            orientation_chart.columns = ["Total Surface Area [m²]"]
            
            st.caption("Hover over bars to check exact surface area contributions for each compass angle row:")
            st.bar_chart(orientation_chart, color="#1f77b4")
        else:
            st.info("No detailed wall components found to extract directional spatial profiles.")
    ...
    with diag_tab2:
        ...
        if base_csv_file and base_csv_file.exists():
            ...
            st.warning("⚠️ Baseline simulation dataset missing. Run baseline workflow initialization to unlock data streaming profiles.")
    ...
    with diag_tab3:
        ...
        if not df_mat.empty:
            ...
            # Highlight conductive thresholds across construction layers interactively
            st.write("##### Layer Conductivity Index")
            chart_thermal = df_thermal.set_index("Material Reference ID")[["Conductivity [W/m·K]"]]
            st.bar_chart(chart_thermal, color="#2ca02c")
        else:
            st.warning("⚠️ No detailed 'MATERIAL' schema blocks found in this building envelope stack.")
